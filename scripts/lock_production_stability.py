...
class ProductionStabilityChecker:
            ...
            try:
                if method == "GET":
                    ...
                    response = requests.post(
                        f"{self.base_url}{endpoint}", json={}, timeout=5
                    )
                elif method == "PUT":
        ...
        try:
            ...
            for table in tables_to_check:
                try:
                    ...
                    self.log(
                        f"❌ Database table {table} check failed: {str(e)}", "ERROR"
                    )
                    all_passed = False
    ...
    def run_all_checks(self) -> Dict[str, Any]:
        ...
        if passed_checks == total_checks:
            ...
            self.log(
                f"❌ {total_checks - passed_checks} checks failed. Fix issues before production deployment."
            )
