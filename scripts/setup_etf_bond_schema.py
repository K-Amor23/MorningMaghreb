...
def migrate_existing_schema(conn):
    ...
    try:
        ...
        if check_table_exists(conn, 'bonds'):
            ...
            with conn.cursor() as cursor:
                cursor.execute("ALTER TABLE bonds ALTER COLUMN ticker TYPE VARCHAR(20);")
                conn.commit()
        ...
        if check_table_exists(conn, 'yield_curve'):
            ...
            with conn.cursor() as cursor:
                cursor.execute("ALTER TABLE yield_curve ALTER COLUMN benchmark_bond TYPE VARCHAR(25);")
                conn.commit()
...
def main():
    ...
    if not conn:
        print("❌ Could not connect to database. Please check your environment variables.")
        sys.exit(1)
