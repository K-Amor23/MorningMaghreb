...
class ProductionReadinessValidator:
    ...
    def __init__(self):
        ...
        self.criteria = {
            ...
            "data_quality": {"min_companies_with_data": 70, "min_excellent_quality": 20},
            "e2e_tests": {"required_tests": ["company", "auth", "data_quality", "charts"]},
            "system_health": {"required_services": ["api", "web", "database", "websocket"]}
        }
    ...
    def validate_production_readiness(self) -> bool:
        ...
        try:
            ...
            validation_results["checks"]["system_health"] = self._validate_system_health()
            
            # 2. API Performance Validation
            validation_results["checks"]["api_performance"] = self._validate_api_performance()
            ...
            validation_results["checks"]["documentation"] = self._validate_documentation()
    ...
    def _check_api_server(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "status": "FAIL",
                "error": f"API server unreachable: {e}"
            }
    ...
    def _check_web_server(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "status": "FAIL",
                "error": f"Web server unreachable: {e}"
            }
    ...
    def _check_database(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "status": "FAIL",
                "error": f"Database health check error: {e}"
            }
    ...
    def _check_file_system(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "status": "FAIL",
                "error": f"File system check failed: {e}"
            }
        ...
        for endpoint in endpoints:
            ...
            if times:
                ...
                performance_data[endpoint] = {
                    ...
                    "status": "PASS" if p95 <= self.criteria["api_performance"]["p95_target"] and success_rate >= self.criteria["api_performance"]["success_rate_target"] else "FAIL"
                }
    ...
    def _validate_data_quality(self) -> Dict[str, Any]:
        ...
        for company in self.companies:
            try:
                response = requests.get(f"{self.api_base_url}/api/companies/{company}/summary", timeout=5)
        ...
        quality_passed = (
            companies_with_data >= self.criteria["data_quality"]["min_companies_with_data"] and
            excellent_count >= self.criteria["data_quality"]["min_excellent_quality"]
        )
    ...
    def _validate_e2e_tests(self) -> Dict[str, Any]:
        ...
        required_tests = ["iam.spec.ts", "atw.spec.ts", "bcp.spec.ts", "auth.spec.ts", "data-quality.spec.ts", "charts.spec.ts"]
        missing_tests = []
    ...
    def _validate_security(self) -> Dict[str, Any]:
        ...
        all_secure = all(check["status"] in ["PASS", "WARN"] for check in security_checks.values())
    ...
    def _check_authentication(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "status": "FAIL",
                "error": f"Auth endpoint unreachable: {e}"
            }
    ...
    def _generate_validation_report(self, results: Dict[str, Any]) -> None:
        ...
        report_file = self.results_dir / f"validation_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
...
"""
    ...
    def _format_validation_summary(self, checks: Dict[str, Any]) -> str:
        ...
        for category, check in checks.items():
            ...
            rows.append(f"| {category.replace('_', ' ').title()} | {status_emoji} {check['status']} | {summary} |")
        ...
        for category, check in checks.items():
            ...
            if "checks" in check:
                for sub_check, sub_result in check["checks"].items():
                    content.append(f"- **{sub_check.replace('_', ' ').title()}**: {sub_result['status']}")
            ...
            if "endpoints" in check:
                ...
                for endpoint, perf in check["endpoints"].items():
                    if perf["status"] == "PASS":
                        ...
                        content.append(f"- ❌ {endpoint}: {perf.get('error', 'Failed')}")
            
            if "stats" in check:
                stats = check["stats"]
                content.append(f"**Data Quality Stats:**")
                content.append(f"- Companies with data: {stats['companies_with_data']}/{stats['total_companies']}")
                content.append(f"- Quality distribution: {stats['quality_distribution']}")
    ...
    def _get_recommendations(self, results: Dict[str, Any]) -> str:
        ...
        if results["overall_status"] == "PASS":
            recommendations.append("✅ **All validation checks passed!** The system is ready for production.")
            recommendations.append("🎯 Proceed with v1.0.0 tagging and baseline metrics capture.")
        else:
            recommendations.append("❌ **Some validation checks failed.** Please address the following issues:")
            
            for category, check in results["checks"].items():
                if check["status"] == "FAIL":
                    recommendations.append(f"- **{category.replace('_', ' ').title()}**: {check.get('error', 'Check failed')}")
            
            recommendations.append("🔧 Fix the issues above before proceeding with production deployment.")
