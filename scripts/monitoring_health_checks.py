...
class HealthMonitor:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        config = {
            ...
            "critical_threshold": int(
                os.getenv("CRITICAL_THRESHOLD", "3")
            ),  # 3 failures
        }
        return config
    ...
    def check_database_tables(self) -> HealthCheck:
        ...
        try:
            ...
            companies_count = (
                len(companies_response.data) if companies_response.data else 0
            )

            market_data_response = (
                supabase.table("market_data").select("count").execute()
            )
            market_data_count = (
                len(market_data_response.data) if market_data_response.data else 0
            )
    ...
    def check_api_endpoints(self) -> HealthCheck:
        ...
        try:
            endpoints = ["/api/companies", "/api/market-data", "/api/macro"]
            ...
            for endpoint in endpoints:
                try:
                    response = requests.get(
                        f"{self.config['web_url']}{endpoint}", timeout=5
                    )
                    if response.status_code == 200:
    ...
    def check_orchestrator_status(self) -> HealthCheck:
        ...
        try:
            ...
            if logs_dir.exists():
                ...
                recent_logs = [
                    f for f in log_files if f.stat().st_mtime > time.time() - 86400
                ]  # Last 24 hours
    ...
    def generate_health_report(self, checks: List[HealthCheck]) -> str:
        ...
        if critical_checks > 0:
            ...
            report_content.append(
                "The following critical issues need immediate attention:"
            )
            for check in status_groups[HealthStatus.CRITICAL]:
                report_content.append(
                    f"- **{check.name}**: Investigate and resolve immediately"
                )
            report_content.append("")
        ...
        if overall_status == HealthStatus.HEALTHY:
            ...
            report_content.append(
                "All systems are healthy. Continue monitoring for any changes."
            )
    ...
    def send_notification(self, checks: List[HealthCheck]):
            ...
            "attachments": [
                {
                    ...
                    "fields": [
                        {"title": check.name, "value": check.message, "short": True}
        ...
        try:
            response = requests.post(
                self.config["notification_webhook"], json=message, timeout=10
            )
