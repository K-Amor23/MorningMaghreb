...
async def run() -> int:
    total = 0
    async with NewsSentimentScraper(batch_size=len(TICKERS), max_concurrent=5) as scraper:
        # Try to fetch real company names from Supabase for better news results
        companies: List[Dict[str, str]] = []
        if create_client and os.getenv("NEXT_PUBLIC_SUPABASE_URL") and os.getenv("SUPABASE_SERVICE_ROLE_KEY"):
            try:
                client = create_client(os.getenv("NEXT_PUBLIC_SUPABASE_URL"), os.getenv("SUPABASE_SERVICE_ROLE_KEY"))
                res = client.table("companies").select("ticker,name").in_("ticker", TICKERS).execute()
                name_map = {row.get("ticker"): row.get("name") for row in (res.data or [])}
                companies = [{"ticker": t, "name": name_map.get(t) or t} for t in TICKERS]
            except Exception:
...
def main() -> None:
    ...
    print("- SELECT ticker, headline, published_at FROM company_news WHERE ticker IN ('ATW','IAM','BCP') ORDER BY published_at DESC LIMIT 20;")
