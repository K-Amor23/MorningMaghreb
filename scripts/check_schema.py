            ...
            if table == "companies":
                ...
                try:
                    result = (
                        ...
                        .select(
                            "ticker,name,sector,industry,market_cap,current_price,price_change,price_change_percent,pe_ratio,dividend_yield,roe,shares_outstanding,size_category,sector_group,exchange,country,company_url,ir_url,is_active"
                        )
                        .limit(1)
                        .execute()
                    )
                    ...
                    try:
                        result = (
                            supabase.table(table)
                            .select("ticker,name,sector,industry,market_cap,is_active")
                            .limit(1)
                            .execute()
                        )
...
def test_insert():
    ...
    try:
        result = (
            ...
            .upsert(test_company, on_conflict="ticker")
            .execute()
        )
