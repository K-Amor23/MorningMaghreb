        ...
        for i, statement in enumerate(statements, 1):
            if statement:
                try:
                    ...
                    logger.info(
                        f"✅ Statement {i}/{len(statements)} executed successfully"
                    )
                except Exception as e:
                    logger.warning(
                        f"⚠️ Statement {i}/{len(statements)} failed (may already exist): {e}"
                    )
                    continue
...
def main():
    ...
    try:
        ...
        # Read SQL file
        sql_file_path = (
            Path(__file__).parent.parent / "database" / "master_pipeline_tables.sql"
        )
