...
class DatabaseMigrator:
    ...
    async def _migrate_data(self) -> bool:
        ...
        for table in tables_to_migrate:
            try:
                ...
                if old_data.data:
                    ...
                    result = self.new_client.table(table).upsert(old_data.data).execute()
        ...
        logger.info(f"✅ Data migration completed. Total records migrated: {total_migrated}")
        return True
    ...
    async def _verify_migration(self) -> bool:
        ...
        try:
            ...
            for table in key_tables:
                try:
                    result = self.new_client.table(table).select('count', count='exact').execute()
            ...
            # Test a simple query
            companies = self.new_client.table('companies').select('ticker, name').limit(5).execute()
            if companies.data:
