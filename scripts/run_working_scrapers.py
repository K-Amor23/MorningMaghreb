...
def sync_companies_to_supabase(companies: List[Dict]):
    ...
    try:
        ...
        for i in range(0, len(companies_to_insert), batch_size):
            ...
            logger.info(
                f"✅ Inserted batch {i//batch_size + 1}: {len(batch)} companies"
            )
...
def generate_price_data_for_companies(companies: List[Dict]):
    ...
    try:
        ...
        for i in range(0, len(price_data), batch_size):
            ...
            total_inserted += len(batch)
            logger.info(
                f"✅ Inserted price batch {i//batch_size + 1}: {len(batch)} records"
            )
...
async def main():
    ...
    try:
        ...
        logger.info(
            f"   ✅ Market Data: {len(market_data.get('tickers', []))} tickers scraped"
        )
        logger.info(
            f"   ✅ Price Data: {'Generated' if prices_generated else 'Failed'}"
        )
        logger.info(
            f"   ✅ Sentiment Data: {'Generated' if sentiment_generated else 'Failed'}"
        )
