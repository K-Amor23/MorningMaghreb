...
class OHLCVDataGenerator:
    ...
    def generate_realistic_price_data(
        self, base_price: float, days: int = 90
    ) -> List[Dict]:
        ...
        for i in range(days - 1, -1, -1):
            ...
            high_price = max(open_price, close_price) * (
                1 + abs(np.random.normal(0, 0.003))
            )
            ...
            volume = int(
                volume_base * volume_multiplier * (0.8 + np.random.random() * 0.4)
            )
    ...
    def generate_company_ohlcv(self, ticker: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"✅ Generated OHLCV data for {ticker}: {len(price_data)} records"
            )
            return True
    ...
    def generate_all_top_20(self) -> bool:
        ...
        logger.info(
            f"✅ Successfully generated data for {success_count}/{len(self.top_20_companies)} companies"
        )
