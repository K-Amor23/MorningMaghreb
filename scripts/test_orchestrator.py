...
class ScraperTester:
    ...
    def test_individual_scrapers(self) -> List[Dict[str, Any]]:
        ...
        for category, scraper_name in test_scrapers:
            try:
                ...
                scraper_class = getattr(__import__(module_path, fromlist=[scraper_name]), scraper_name)
                ...
                if test_result["status"] == "success":
                    print(f"  ✅ {scraper_name}: {len(data)} records, validation passed")
                else:
    ...
    def test_data_validation(self) -> List[Dict[str, Any]]:
        ...
        try:
            from scrapers.utils.data_validators import validate_dataframe, clean_dataframe
            import pandas as pd
    ...
    def generate_test_report(self) -> str:
        ...
        for test_type, tests in test_groups.items():
            ...
            for test in tests:
                ...
                report_content.append(f"- {status_emoji} **{test['name']}** - {test['status']}")
                ...
                if "data_info" in test:
                    ...
                    report_content.append(f"  - Data: {data_info['rows']} rows, {len(data_info['columns'])} columns")
        ...
        if self.results["summary"]["failed"] > 0:
            ...
            report_content.append("The following tests failed and need immediate attention:")
            for test in all_tests:
                if test["status"] == "failed":
                    report_content.append(f"- **{test['name']}**: {test.get('error', 'Unknown error')}")
            report_content.append("")
