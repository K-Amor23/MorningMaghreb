...
# Morning Maghreb Automated Data Collection Setup
# This script sets up cron jobs for automated data collection

set -e

echo "🚀 Setting up Morning Maghreb Automated Data Collection..."
echo "=========================================================="
...
cat > scripts/collect_market_data.py << 'EOF'
#!/usr/bin/env python3
"""
...
chmod +x scripts/collect_market_data.py

# Create logs directory
mkdir -p logs

# Create cron job for daily data collection at 6:00 AM
echo "⏰ Setting up cron job for daily data collection..."

# Check if cron job already exists
if crontab -l 2>/dev/null | grep -q "collect_market_data.py"; then
    echo "✅ Cron job already exists"
else
    # Add cron job
    (crontab -l 2>/dev/null; echo "0 6 * * * cd $(pwd) && ./scripts/collect_market_data.py >> logs/cron.log 2>&1") | crontab -
    echo "✅ Cron job added for daily data collection at 6:00 AM"
fi

# Create a manual test script
cat > scripts/test_data_collection.sh << 'EOF'
#!/bin/bash
...
echo ""
echo "🎉 AUTOMATED DATA COLLECTION SETUP COMPLETE"
echo "==========================================="
echo "📊 Data Collection: Daily at 6:00 AM"
echo "🗄️ Database: https://gzsgehciddnrssuqxtsj.supabase.co"
echo "📝 Logs: logs/data_collection.log"
echo "🧪 Test: ./scripts/test_data_collection.sh"
echo "==========================================="
...
echo ""
echo "📋 Next Steps:"
echo "1. Test the data collection: ./scripts/test_data_collection.sh"
echo "2. Check the logs: tail -f logs/data_collection.log"
echo "3. View cron jobs: crontab -l"
echo "4. Monitor daily runs in the logs directory"

echo ""
echo "🛠️ Useful Commands:"
echo "- Test collection: ./scripts/test_data_collection.sh"
echo "- View logs: tail -f logs/data_collection.log"
echo "- List cron jobs: crontab -l"
echo "- Remove cron job: crontab -r (then re-add if needed)" 