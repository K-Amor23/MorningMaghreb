...
class LiveTickerOrchestrator:
    ...
    async def scrape_african_markets_live(self) -> List[LiveTickerData]:
        ...
        try:
            ...
            async with AfricanMarketsScraper() as scraper:
                ...
                logger.info(f"✅ Collected live data for {len(live_data)} priority companies from African Markets")
                return live_data
    ...
    def scrape_casablanca_bourse_live(self) -> List[LiveTickerData]:
        ...
        try:
            ...
            logger.info(f"✅ Collected live data for {len(live_data)} priority companies from Casablanca Bourse")
            return live_data
    ...
    def scrape_wafa_bourse_live(self) -> List[LiveTickerData]:
        ...
        try:
            ...
            logger.info(f"✅ Collected live data for {len(live_data)} priority companies from Wafa Bourse")
            return live_data
    ...
    def merge_live_data(self, data_sources: List[List[LiveTickerData]]) -> List[LiveTickerData]:
        ...
        for source_data in data_sources:
            for ticker_data in source_data:
                ...
                if ticker not in merged_data or ticker_data.timestamp > merged_data[ticker].timestamp:
            ...
            for ticker_data in live_data:
                try:
                    ...
                    if response.status_code in [200, 201]:
                        logger.info(f"✅ Updated live ticker {ticker_data.ticker} in database")
                        updated_count += 1
                    else:
                        logger.warning(f"⚠️ Failed to update {ticker_data.ticker}: {response.status_code}")
    ...
    def print_live_summary(self, live_data: List[LiveTickerData]):
        ...
        # Show top movers
        gainers = [d for d in live_data if d.change_1d_percent and d.change_1d_percent > 0]
        losers = [d for d in live_data if d.change_1d_percent and d.change_1d_percent < 0]
        
        if gainers:
            logger.info(f"\n📈 Top Gainers:")
            for ticker in sorted(gainers, key=lambda x: x.change_1d_percent or 0, reverse=True)[:5]:
                logger.info(f"  {ticker.ticker}: {ticker.change_1d_percent:.2f}% (${ticker.price:.2f})")
    ...
    async def run_live_update(self):
        ...
        try:
            logger.info(f"🚀 Starting live ticker update at {datetime.now().strftime('%H:%M:%S')}")
