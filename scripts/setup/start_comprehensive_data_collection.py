...
async def start_comprehensive_data_collection():
    ...
    try:
        ...
        async with ComprehensiveMarketScraper() as scraper:
            ...
            logger.info(
                f"  - Market data: {len(comprehensive_data.get('market_data', []))} companies"
            )
            logger.info(f"  - News: {len(comprehensive_data.get('news', []))} items")
            logger.info(
                f"  - Dividends: {len(comprehensive_data.get('dividends', []))} announcements"
            )
            logger.info(
                f"  - Earnings: {len(comprehensive_data.get('earnings', []))} announcements"
            )
            logger.info(f"  - ETFs: {len(comprehensive_data.get('etfs', []))} ETFs")
            logger.info(
                f"  - Corporate actions: {len(comprehensive_data.get('corporate_actions', []))} actions"
            )
            logger.info(
                f"  - Market sentiment: {len(comprehensive_data.get('sentiment', []))} sentiment scores"
            )
            ...
            json_file, csv_file = await scraper.export_comprehensive_data(
                comprehensive_data, output_dir
            )
            ...
            if comprehensive_data.get("market_status"):
                await scraper.populate_market_status(
                    comprehensive_data["market_status"]
                )
                logger.info("✅ Market status populated")

            # Populate comprehensive market data
            if comprehensive_data.get("market_data"):
                await scraper.populate_comprehensive_market_data(
                    comprehensive_data["market_data"]
                )
                logger.info(
                    f"✅ Market data for {len(comprehensive_data['market_data'])} companies populated"
                )
            ...
            if comprehensive_data.get("dividends"):
                await scraper.populate_dividend_announcements(
                    comprehensive_data["dividends"]
                )
                logger.info(f"✅ Dividend announcements populated")

            # Populate earnings announcements
            if comprehensive_data.get("earnings"):
                await scraper.populate_earnings_announcements(
            ...
            if comprehensive_data.get("corporate_actions"):
                await scraper.populate_corporate_actions(
                    comprehensive_data["corporate_actions"]
                )
            ...
            logger.info(
                "🎉 Comprehensive data collection and database population completed!"
            )
            logger.info(
                "🌐 Your enhanced frontend now has access to real company profiles!"
            )
