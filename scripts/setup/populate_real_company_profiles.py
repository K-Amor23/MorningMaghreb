...
async def populate_real_company_profiles():
    ...
    try:
        ...
        logger.info(
            "🎉 Successfully populated Sky Garden database with real company profiles!"
        )
        return True
        ...
        for company in companies:
            try:
                ...
                market_record = {
                    ...
                    "low": float(price),
                    "volume": int(
                        company.get("volume") or random.randint(1000, 100000)
                    ),
                    "market_cap": float(
                        (company.get("market_cap_billion") or 0.0) * 1e9
                    ),
                    ...
                    "avg_volume": int(
                        company.get("volume") or random.randint(5000, 50000)
                    ),
                ...
                logger.debug(
                    f"Error processing company {company.get('ticker', 'Unknown')}: {e}"
                )
                continue
        ...
        for company in companies[:15]:  # Limit to first 15 companies
            ...
            for i in range(random.randint(1, 2)):
                ...
                if dividend_type == "dividend":
                    amount = round(
                        price * random.uniform(0.02, 0.08), 2
                    )  # 2-8% of price
                ...
                announcement_date = datetime.now() - timedelta(
        ...
        for company in companies[:20]:  # Limit to first 20 companies
            ...
            for period in random.sample(periods, random.randint(2, 4)):
                ...
                if status == "reported":
                    ...
                    surprise_percent = (
                        (surprise / estimate) * 100 if estimate != 0 else 0
                    )
                else:
                    actual = None
                    surprise = None
