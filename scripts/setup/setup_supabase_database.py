...
class SupabaseDatabaseSetup:
            ...
            for i, statement in enumerate(statements):
                if statement:
                    try:
                        ...
                        self.print_status(
                            f"Statement {i+1}/{len(statements)} completed", "SUCCESS"
                        )
                    except Exception as e:
                        # Try direct execution for some statements
                        try:
                            result = (
                                self.supabase.table("_dummy")
                                .select("*")
                                .limit(1)
                                .execute()
                            )
                            self.print_status(
                                f"Statement {i+1}/{len(statements)} completed (alternative method)",
                                "SUCCESS",
                            )
                        except:
                            self.print_status(
                                f"Statement {i+1}/{len(statements)} failed: {str(e)}",
                                "WARNING",
                            )
    ...
    async def setup_database_schema(self) -> bool:
        ...
        schema_file = (
            Path(__file__).parent.parent / "database" / "enhanced_schema_with_rls.sql"
        )
        ...
        if not schema_file.exists():
            ...
            if not schema_file:
                self.print_status(
                    f"Schema file not found. Tried: enhanced_schema_with_rls.sql, advanced_features_schema.sql, supabase_financial_schema.sql, schema.sql",
                    "ERROR",
                )
                return False
    ...
    async def test_database_connectivity(self) -> bool:
        ...
        try:
            ...
            result = (
                self.supabase.table("newsletter_subscribers")
                .select("*")
                .limit(1)
                .execute()
            )
            ...
            self.print_status(
                f"❌ Database connectivity test failed: {str(e)}", "ERROR"
            )
            return False
        ...
        try:
            ...
            for company in companies_data:
                try:
                    ...
                    self.print_status(
                        f"Warning: Could not insert company {company['ticker']}: {str(e)}",
                        "WARNING",
                    )
            ...
            try:
                self.supabase.table("newsletter_templates").upsert(
                    template_data
                ).execute()
                self.print_status("✅ Newsletter template inserted", "SUCCESS")
            except Exception as e:
                self.print_status(
                    f"Warning: Could not insert newsletter template: {str(e)}",
                    "WARNING",
                )
    ...
    async def setup_realtime_subscriptions(self) -> bool:
        ...
        try:
            ...
            self.print_status(
                "✅ Real-time subscriptions ready (configure in dashboard)", "SUCCESS"
            )
            return True
    ...
    async def run_complete_setup(self) -> bool:
        ...
        for step_name, step_func in steps:
            ...
            try:
                ...
                self.print_status(
                    f"❌ {step_name} failed with error: {str(e)}", "ERROR"
                )
        ...
        for step_name, result in results:
            ...
            self.print_status(
                f"{step_name}: {status}", "SUCCESS" if result else "ERROR"
            )
            if result:
                passed += 1

        self.print_status(
            f"Overall: {passed}/{total} steps passed",
