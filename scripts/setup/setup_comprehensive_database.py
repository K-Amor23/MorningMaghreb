...
class ComprehensiveDatabaseSetup:
        ...
        try:
            ...
            async with self.session.post(url, json=payload) as response:
                if response.status == 200:
                    ...
                    logger.error(
                        f"❌ {description} failed: {response.status} - {error_text}"
                    )
                    return False
    ...
    async def create_indexes(self) -> bool:
        ...
        indexes = [
            (
                ...
                "Market status scraped_at index",
            ),
    ...
    async def setup_complete_database(self) -> bool:
        ...
        setup_steps = [
            ...
            (
                "Creating comprehensive market data table",
                self.create_comprehensive_market_data_table,
            ),
            ("Creating company news table", self.create_company_news_table),
            (
                "Creating dividend announcements table",
                self.create_dividend_announcements_table,
            ),
            (
                "Creating earnings announcements table",
                self.create_earnings_announcements_table,
            ),
...
async def main():
    ...
    if not supabase_url or not supabase_key:
        logger.error(
            "❌ Missing Supabase credentials. Please provide --supabase-url and --supabase-key or set environment variables."
        )
        sys.exit(1)
