...
class ProductionDeployer:
    ...
    def __init__(self, config: ProductionConfig):
        self.config = config
        self.deployment_id = (
            f"prod_deployment_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
        self.deployment_log = []
    ...
    async def deploy_to_production(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"✅ Production deployment {self.deployment_id} completed successfully"
            )
    ...
    async def _verify_production_access(self) -> bool:
        ...
        try:
            ...
            response = requests.get(
                f"{self.config.supabase_url}/rest/v1/", headers=headers, timeout=10
            )
    ...
    async def _verify_migration_files(self) -> bool:
        ...
        migration_file = (
            self.config.migrations_dir / "migration_001_indexes_constraints.sql"
        )
    ...
    async def _create_production_backup(self) -> bool:
        ...
        try:
            backup_file = (
                self.config.backup_dir / f"prod_backup_{self.deployment_id}.sql"
            )
            ...
            cmd = [
                "pg_dump",
                "--host",
                self.config.database_url.split("@")[1].split(":")[0],
                "--port",
                "5432",
                "--username",
                "postgres",
            ...
            if result.returncode == 0:
                ...
                logger.info(
                    f"✅ Production backup created: {backup_file} ({backup_size:.2f}MB)"
                )
                return True
            else:
    ...
    async def _run_production_migrations(self) -> bool:
        ...
        try:
            ...
            # Execute migration
            success = await self._execute_production_sql(
                migration_sql, migration_file.name
            )
    ...
    async def _verify_schema_changes(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                missing_indexes = [
                    idx for idx in expected_indexes if idx not in index_names
                ]
        ...
        try:
            ...
            for table in tables_to_check:
                ...
                if response.status_code == 200:
                    ...
                    logger.error(
                        f"❌ Could not check table {table}: {response.status_code}"
                    )
                    return False
            ...
            for endpoint in endpoints:
                ...
                for i in range(5):
                    ...
                    if response.status_code == 200:
                        ...
                        logger.error(
                            f"❌ Performance test failed for {endpoint}: {response.status_code}"
                        )
                        return False
                ...
                if avg_response_time > 500:
                    logger.warning(
                        f"⚠️  Slow response time for {endpoint}: {avg_response_time:.2f}ms"
                    )

            # Save performance baseline
            baseline_file = (
                self.config.backup_dir
                / f"performance_baseline_{self.deployment_id}.json"
            )
