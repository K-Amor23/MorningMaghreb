...
class ComprehensiveDataCollector:
    ...
    async def collect_african_markets_data(self):
        ...
        try:
            logger.info("🔍 Collecting data for all 78 companies from African Markets...")
            ...
            async with AfricanMarketsScraper() as scraper:
                ...
                if companies:
                    ...
                    logger.info(f"✅ Collected data for {len(companies)} companies from African Markets")
                    
                    # Save to file
                    output_file = self.output_dir / f"african_markets_data_{self.timestamp}.json"
                    ...
                    self.results["metadata"]["errors"].append("No companies found from African Markets")
                    
        except Exception as e:
            logger.error(f"❌ Error collecting African Markets data: {e}")
            self.results["metadata"]["errors"].append(f"African Markets error: {str(e)}")
    ...
    def collect_casablanca_bourse_data(self):
        ...
        try:
            ...
            if market_data:
                ...
                # Save to file
                output_file = self.output_dir / f"casablanca_bourse_data_{self.timestamp}.json"
                with open(output_file, 'w') as f:
                    json.dump(market_data, f, indent=2)
                logger.info(f"💾 Saved to: {output_file}")
            else:
                logger.warning("⚠️ No Casablanca Bourse data found")
                self.results["metadata"]["errors"].append("No Casablanca Bourse data found")
                
        except Exception as e:
            logger.error(f"❌ Error collecting Casablanca Bourse data: {e}")
            self.results["metadata"]["errors"].append(f"Casablanca Bourse error: {str(e)}")
    ...
    def collect_bank_al_maghrib_data(self):
        ...
        try:
            ...
            if banking_data:
                ...
                # Save to file
                output_file = self.output_dir / f"bank_al_maghrib_data_{self.timestamp}.json"
                with open(output_file, 'w') as f:
                    json.dump(banking_data, f, indent=2)
                logger.info(f"💾 Saved to: {output_file}")
            else:
                logger.warning("⚠️ No Bank Al Maghrib data found")
                self.results["metadata"]["errors"].append("No Bank Al Maghrib data found")
                
        except Exception as e:
            logger.error(f"❌ Error collecting Bank Al Maghrib data: {e}")
            self.results["metadata"]["errors"].append(f"Bank Al Maghrib error: {str(e)}")
        ...
        try:
            ...
            for company in self.results.get("companies", []):
                try:
                    ...
                    if response.status_code in [200, 201]:
                        logger.info(f"✅ Updated {company.get('ticker', 'Unknown')} in database")
                    else:
                        logger.warning(f"⚠️ Failed to update {company.get('ticker', 'Unknown')}: {response.status_code}")
                except Exception as e:
                    logger.error(f"❌ Error updating company {company.get('ticker', 'Unknown')}: {e}")
            
            # Update bonds
            for bond in self.results.get("bonds", []):
                try:
                    response = requests.post(
                        f"{SUPABASE_URL}/rest/v1/bonds",
                        headers=headers,
                        json=bond
                    )
                    if response.status_code in [200, 201]:
                        logger.info(f"✅ Updated bond {bond.get('ticker', 'Unknown')} in database")
                    else:
                        logger.warning(f"⚠️ Failed to update bond {bond.get('ticker', 'Unknown')}: {response.status_code}")
                except Exception as e:
                    logger.error(f"❌ Error updating bond {bond.get('ticker', 'Unknown')}: {e}")
            ...
            for etf in self.results.get("etfs", []):
                try:
                    ...
                    if response.status_code in [200, 201]:
                        logger.info(f"✅ Updated ETF {etf.get('ticker', 'Unknown')} in database")
                    else:
                        logger.warning(f"⚠️ Failed to update ETF {etf.get('ticker', 'Unknown')}: {response.status_code}")
                except Exception as e:
                    logger.error(f"❌ Error updating ETF {etf.get('ticker', 'Unknown')}: {e}")
    ...
    def save_comprehensive_results(self):
        ...
        try:
            output_file = self.output_dir / f"comprehensive_market_data_{self.timestamp}.json"
            with open(output_file, 'w') as f:
    ...
    def print_summary(self):
        ...
        logger.info(f"🏦 Banking Data: {'✅' if self.results['banking_data'] else '❌'}")
