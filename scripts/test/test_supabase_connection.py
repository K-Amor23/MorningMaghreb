...
def test_supabase_connection():
    ...
    try:
        ...
        try:
            ...
            print_status(
                f"⚠️ Companies table not found (this is normal if schema not set up): {str(e)}",
                "WARNING",
            )
        ...
        try:
            ...
            print_status(
                f"⚠️ Newsletter subscribers table not found: {str(e)}", "WARNING"
            )
...
def test_sample_data_insertion():
    ...
    try:
        ...
        try:
            ...
            print_status(
                f"⚠️ Could not insert sample company (table may not exist): {str(e)}",
                "WARNING",
            )
...
def main():
    ...
    if data_success:
        ...
        print_status(
            "⚠️ Data Insertion Test: PARTIAL (tables may not exist yet)", "WARNING"
        )
    ...
    if connection_success:
        ...
        print_status(
            "❌ Supabase connection failed. Check your environment variables.", "ERROR"
        )
