...
async def test_welcome_email():
    ...
    success = await send_welcome_email(email="test@example.com", name="Test User")
...
async def test_password_reset():
    ...
    success = await send_password_reset(
        email="test@example.com", reset_token=reset_token
    )
...
async def test_bulk_email():
    ...
    test_subscribers = ["test1@example.com", "test2@example.com", "test3@example.com"]
...
async def test_email_providers():
    ...
    print_status(
        f"SendGrid API Key: {'✅ Set' if sendgrid_key else '❌ Not set'}", "INFO"
    )
    print_status(f"Supabase URL: {'✅ Set' if supabase_url else '❌ Not set'}", "INFO")
    print_status(
        f"Supabase Service Key: {'✅ Set' if supabase_key else '❌ Not set'}", "INFO"
    )
...
async def run_all_tests():
    ...
    if passed == total:
        print_status(
            "🎉 All email tests passed! Email service is working correctly.", "SUCCESS"
        )
    else:
...
def main():
    ...
    if not os.path.exists("apps/backend/lib/email_service.py"):
        print_status(
            "❌ Please run this script from the project root directory", "ERROR"
        )
        sys.exit(1)
