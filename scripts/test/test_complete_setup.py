...
class CompleteSetupTest:
    ...
    def test_environment_variables(self):
        ...
        if missing_optional:
            self.print_status(f"Optional variables missing: {', '.join(missing_optional)}", "WARNING")
    ...
    def run_all_tests(self):
        ...
        self.print_status(f"Overall: {passed}/{total} tests passed", "SUCCESS" if passed == total else "WARNING")
        
        if passed == total:
            self.print_status("🎉 All tests passed! Setup is complete and ready.", "SUCCESS")
        else:
            self.print_status("⚠️ Some tests failed. Please check the issues above.", "WARNING")
