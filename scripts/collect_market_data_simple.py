...
def collect_mock_market_data():
    ...
    try:
        ...
        logger.info(
            f"✅ Mock market data collected: {len(mock_data['companies'])} companies"
        )
        return mock_data
    ...
    try:
        ...
        for company in data.get("companies", []):
            ...
            response = requests.post(
                f"{SUPABASE_URL}/rest/v1/companies", headers=headers, json=company_data
            )
            ...
            if response.status_code in [200, 201]:
                ...
                logger.warning(
                    f"⚠️ Failed to update {company['ticker']}: {response.status_code}"
                )
        ...
        if summary_data:
            ...
            if response.status_code in [200, 201]:
                ...
                logger.warning(
                    f"⚠️ Failed to update market summary: {response.status_code}"
                )
