...
class PerformanceTester:
    ...
    def run_basic_performance_tests(self) -> Dict[str, Any]:
        ...
        for endpoint_name, endpoint_path in endpoints:
            ...
            logger.info(
                f"✅ {endpoint_name}: {avg_time:.2f}ms avg, {p95_time:.2f}ms p95, {success_rate:.1f}% success"
            )
    ...
    def _test_endpoint(self, endpoint_name: str, endpoint_path: str) -> Dict[str, Any]:
        ...
        for ticker in self.companies:
            ...
            try:
                ...
                results["total_requests"] += 1
                results["errors"].append(
                    {"ticker": ticker, "status_code": None, "error": str(e)}
                )

        # Calculate statistics
        if results["response_times"]:
            results["avg_response_time_ms"] = sum(results["response_times"]) / len(
            ...
            results["p99_response_time_ms"] = sorted(results["response_times"])[
    ...
    def _calculate_summary(self, endpoints: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if all_response_times:
            summary = {
                ...
                "p99_response_time_ms": sorted(all_response_times)[
...
'''
    ...
    def run_load_test(
        self, users: int = 10, spawn_rate: int = 2, run_time: str = "5m"
    ) -> bool:
        """Run Locust load test"""
        logger.info(
            f"🚀 Starting Locust load test: {users} users, {spawn_rate}/s spawn rate, {run_time} duration"
        )
    ...
    def analyze_slow_queries(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"❌ Failed to analyze slow queries: {response.status_code}"
                )
        ...
        for query in slow_queries:
            ...
            if "companies" in query_text.lower() and "ticker" in query_text.lower():
                recommendations.append(
                    f"Consider adding index on companies(ticker) for query taking {mean_time:.2f}ms"
                )

            if "company_prices" in query_text.lower() and "date" in query_text.lower():
                recommendations.append(
                    f"Consider adding index on company_prices(ticker, date) for query taking {mean_time:.2f}ms"
                )

            if "company_reports" in query_text.lower():
                recommendations.append(
                    f"Consider adding index on company_reports(ticker, report_type) for query taking {mean_time:.2f}ms"
                )

            if "company_news" in query_text.lower():
                recommendations.append(
                    f"Consider adding index on company_news(ticker, published_at) for query taking {mean_time:.2f}ms"
                )
...
def get_casablanca_companies() -> List[str]:
    ...
    if len(unique_companies) < 78:
        logger.warning(
            f"⚠️  Only {len(unique_companies)} unique companies found, adding duplicates"
        )
        while len(unique_companies) < 78:
            unique_companies.append(
                unique_companies[len(unique_companies) % len(unique_companies)]
            )
...
async def main():
    ...
    basic_results_file = (
        results_dir
        / f"basic_performance_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
    ...
    try:
        ...
        locust_success = locust_tester.run_load_test(
            users=20, spawn_rate=5, run_time="10m"
        )
    ...
    if supabase_url and supabase_key:
        ...
        query_analysis_file = (
            results_dir
            / f"query_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        ...
        logger.warning(
            "⚠️  Skipping database query analysis (missing Supabase credentials)"
        )
    ...
    if summary["p95_response_time_ms"] <= p95_target:
        logger.info(
            f"✅ P95 response time ({summary['p95_response_time_ms']:.2f}ms) meets target ({p95_target}ms)"
        )
    else:
        logger.warning(
            f"⚠️  P95 response time ({summary['p95_response_time_ms']:.2f}ms) exceeds target ({p95_target}ms)"
        )

    if summary["overall_success_rate"] >= 99.0:
        logger.info(
            f"✅ Success rate ({summary['overall_success_rate']:.1f}%) meets target (99.0%)"
        )
    else:
        logger.warning(
            f"⚠️  Success rate ({summary['overall_success_rate']:.1f}%) below target (99.0%)"
        )
