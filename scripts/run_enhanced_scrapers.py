...
async def scrape_news_sentiment(companies: List[Dict]):
    ...
    try:
        ...
        return {"companies_processed": companies_processed, "total_news": total_news}
...
def sync_data_to_supabase(
    ...
    try:
        ...
        if financial_reports_results:
            logger.info(
                f"📄 Syncing {financial_reports_results['total_reports']} financial reports to Supabase"
            )
        ...
        if news_sentiment_results:
            logger.info(
                f"📰 Syncing {news_sentiment_results['total_news']} news items to Supabase"
            )
        ...
        return {
            ...
            "market_data_synced": bool(
                casablanca_bourse_results
                and casablanca_bourse_results.get("market_data")
            ),
            "reports_synced": (
                financial_reports_results.get("total_reports", 0)
                if financial_reports_results
                else 0
            ),
            "news_synced": (
                news_sentiment_results.get("total_news", 0)
                if news_sentiment_results
                else 0
            ),
        }
...
async def main():
    ...
    try:
        ...
        logger.info(
            f"   ✅ Market Data: {casablanca_bourse_results['tickers_count']} tickers"
        )
        logger.info(
            f"   ✅ Financial Reports: {financial_reports_results['total_reports']}"
        )
        logger.info(f"   ✅ News Items: {news_sentiment_results['total_news']}")
        logger.info(
            f"   ✅ Synced to Supabase: {sync_results['companies_synced']} companies"
        )
